...
class DashboardController {
    ...
    private function getRecentActivities() {
        ...
        $query = "SELECT 
                 ...
                 WHERE 1=1
                   AND mt.triwulan IS NOT NULL
                   AND mt.tahun IS NOT NULL
                   $fakultasCondition
        ...
        $activities = [];
        while ($row = $result->fetch_assoc()) {
            // Pastikan data `triwulan` dan `tahun` valid
            if (isset($row['triwulan']) && isset($row['tahun'])) {
                $activities[] = [
                    'risk_id' => $row['risk_id'],
                    'risk_event' => $row['risk_event'],
                    'likelihood_inherent' => $row['likelihood_inherent'],
                    'impact_inherent' => $row['impact_inherent'],
                    'risk_level_inherent' => $row['risk_level_inherent'],
                    'rencana_mitigasi' => $row['rencana_mitigasi'],
                    'triwulan' => $row['triwulan'],
                    'tahun' => $row['tahun'],
                    'kategori_nama' => $row['kategori_nama'],
                    'fakultas_nama' => $row['fakultas_nama'],
                    'last_update' => $row['last_update']
                ];
            } else {
                error_log("Invalid timeline data for risk ID: " . $row['risk_id']);
            }
        }
        return $activities;
    }     
