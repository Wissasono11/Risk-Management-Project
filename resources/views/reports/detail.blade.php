...
<div class="reports-container">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= htmlspecialchars($detailTotalRisks ?? 0) ?></div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-value"><?= htmlspecialchars($detailHighRisks ?? 0) ?></div>
        </div>

        <div class="stat-card">
            <div class="stat-header">
                <div class="stat-icon" style="background: rgba(124, 58, 237, 0.1); color: var(--color-very-high)">
            ...
            <div class="stat-value">
                <?php 
                $progress = ($detailTotalTreatments ?? 0) > 0 
    ...
    <div class="risk-table-container">
        <table class="table">
            ...
            <tbody>
                ...
                <?php 
                $level = calculateRiskLevel($risk['likelihood_inherent'] ?? 0, $risk['impact_inherent'] ?? 0);
                ...
                ?>
                    <tr>
                        <td>
                            <div class="risk-info">
                                <span class="risk-title"><?= htmlspecialchars($risk['risk_event'] ?? 'N/A') ?></span>
                                <small class="risk-desc"><?= htmlspecialchars($risk['objective'] ?? 'N/A') ?></small>
                            </div>
                        </td>
                        <td><?= htmlspecialchars($risk['kategori_nama'] ?? 'N/A') ?></td>
                        <td>
                            <span class="risk-badge" 
                                  style="color: <?= $level['color'] ?>; 
                                         background-color: <?= $level['background'] ?>;">
                                <?= htmlspecialchars($level['label'] ?? 'N/A') ?>
                            </span>
                        </td>
                        <td><?= htmlspecialchars($risk['risk_owner'] ?? 'N/A') ?></td>
                        <td>
                            <div class="progress">
                                <div class="progress-bar" style="width: <?= $progress2 ?>%">
                                    <span class="progress-value"><?= htmlspecialchars($progress2 . '%') ?></span>
                                </div>
                            </div>
                        </td>
                        <td>
